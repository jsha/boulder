...
func (ca *CertificateAuthorityImpl) IssueCertificate(csr x509.CertificateRequest, regID int64) (core.Certificate, error) {
	...
	// serialSeq we provide, but we want 128 bits of random number, plus an 8-bit
	// instance id prefix. For now, we generate the extra 64 bits of randomness
	// ourselves. We should modify CFSSL to just allow the caller to select its
